    ...
    <div>
      ...
      <div className="flex justify-center gap-4 mb-8">
        <button
          ...
          className={`px-8 py-2.5 rounded-lg text-lg font-semibold transition-all duration-200 border-2 ${criteria.mediaType === 'photo' ? 'bg-red-500 border-red-500 text-white' : 'bg-gray-700 border-gray-600 hover:border-red-500'}`}
        >
          Fotografie
        </button>
        <button
          onClick={() => setCriteria(prev => ({ ...prev, mediaType: 'video' }))}
          className={`px-8 py-2.5 rounded-lg text-lg font-semibold transition-all duration-200 border-2 ${criteria.mediaType === 'video' ? 'bg-red-500 border-red-500 text-white' : 'bg-gray-700 border-gray-600 hover:border-red-500'}`}
        >
          Videografie
        </button>
      </div>
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center p-4 rounded-lg border-2 transition-all duration-200 text-center
                ${isSelected ? 'bg-red-500/20 border-red-500 text-white' : 'bg-gray-700/50 border-gray-600 hover:border-red-500 hover:bg-gray-700 text-gray-300'}
              `}
            >
